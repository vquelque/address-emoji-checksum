 ...
 */
const generateEmojiChecksum = async (
  address: string,
  length: number = 4,
  network?: Network,
  ...
  const emojis = [...Array(length)].map((_, i) => {
    const segment = hash.slice(i * (hash.length / length), (i + 1) * (hash.length / length))
...
export { validateAddress, generateEmojiChecksum, Network }
